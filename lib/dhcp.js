...
Server.prototype = {
  ...
  _getLease: function(clientMAC, callback) {
    var lease = this._state[clientMAC];

    if (lease) {
      return callback(undefined, lease);
    }

    this._selectAddress(clientMAC, (err, address) => {
      if (err) { return callback(err); }

      lease = this._state[clientMAC] = new Lease;
  ...
  _selectAddress: function(clientMAC, callback) {
    if (this._conf.selectAddress) {
      return this._conf.selectAddress(clientMAC, callback);
    }
    /*
    ...
    if (this._state[clientMAC] && this._state[clientMAC].address) {
      return callback(undefined, this._state[clientMAC].address);
    }
    ...
    if (static[clientMAC]) {
      return callback(undefined, static[clientMAC]);
    }
  ...
  handleDiscover: function(req) {
    ...
    this._getLease(req.chaddr, (err, lease) => {
      if (err) { return console.error('handleDiscover', err); }
      lease.state = 'OFFERED';
  ...
  sendOffer: function(req) {
    ...
    var lease = this._state[req.chaddr];
  ...
  handleRequest: function(req) {
    ...
    this._getLease(req.chaddr, (err, lease) => {
      if (err) { return console.error('handleRequest', err); }
